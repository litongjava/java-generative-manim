...
class CombinedScene(MovingCameraScene):
    ...
    def play_scene_01(self):
        ...
        for _ in range(num_stars):
            ...
            star.add_updater(
                lambda m, dt, base=star.get_fill_opacity(), amp=amplitude, freq=frequency, ph=phase: m.set_opacity(
    ...
    def play_scene_02(self):
        ...
        dot.add_updater(
            lambda m: m.scale(1 + 0.1 * np.sin(pulse_tracker.get_value() * 4 * PI), about_point=m.get_center()))
    ...
    def play_scene_03(self):
        ...
        tangent_line = axes3.plot(tangent_line_func, x_range=[-2, 3], color=MY_ORANGE,
                                  stroke_width=2.5)  # 限制 x_range 使线不过长
    ...
    def play_scene_04(self):
        ...
        # 点斜式方程
        pointslope_title = Text("理论基础 2: 点斜式方程", font_size=32, color=BLACK).to_edge(DOWN, buff=2.0).shift(
            LEFT * 3)
